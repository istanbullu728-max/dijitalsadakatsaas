...
const I = "#6366F1", P = "#8B5CF6", D = "#0F172A", G = "#64748B", L = "#F8FAFC", B = "#E2E8F0";
...
function PhoneMockup({ stamps }: { stamps: number }) {
  return (
    <div style={{ position:"relative", display:"flex", justifyContent:"center" }}>
      ...
      <div style={{ width:240, background:"#1E1B4B", borderRadius:40, padding:"12px 8px 8px", boxShadow:"0 40px 80px rgba(99,102,241,0.3), 0 0 0 8px rgba(255,255,255,0.05)", position:"relative", zIndex:1 }}>
        <div style={{ background:"linear-gradient(145deg,#6366F1,#4338CA)", borderRadius:28, overflow:"hidden" }}>
          <div style={{ padding:"1.25rem 1rem 0.75rem" }}>
            <div style={{ fontSize:"0.45rem", fontWeight:700, color:"rgba(255,255,255,0.5)", letterSpacing:"0.15em" }}>SADAKAT KARTI</div>
            <div style={{ fontSize:"1rem", fontWeight:800, color:"white" }}>Coffee Co.</div>
            <div style={{ fontSize:"0.5rem", color:"rgba(255,255,255,0.55)", marginTop:2 }}>1 Bedava Kahve</div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginTop:"0.75rem" }}>
              {Array.from({length:7}).map((_,i) => (
                <div key={i} style={{ width:22, height:22, borderRadius:"50%", border:`1.5px solid rgba(255,255,255,${i<stamps?0:0.25})`, background: i<stamps?"rgba(255,255,255,0.95)":"rgba(255,255,255,0.05)", display:"flex", alignItems:"center", justifyContent:"center", transition:"all 0.4s" }}>
                  {i<stamps && <svg width="10" height="10" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7" stroke="#6366F1" strokeWidth="3" strokeLinecap="round" fill="none"/></svg>}
                </div>
              ))}
            </div>
            <div style={{ display:"flex", justifyContent:"space-between", marginTop:"0.75rem", paddingBottom:"0.5rem" }}>
              <div><div style={{ fontSize:"0.4rem", fontWeight:700, color:"rgba(255,255,255,0.4)", letterSpacing:"0.1em" }}>DAMGA</div><div style={{ fontSize:"0.7rem", fontWeight:700, color:"white" }}>{stamps} / 7</div></div>
              <div style={{ textAlign:"right" }}><div style={{ fontSize:"0.4rem", fontWeight:700, color:"rgba(255,255,255,0.4)", letterSpacing:"0.1em" }}>MÜŞTERİ</div><div style={{ fontSize:"0.7rem", fontWeight:700, color:"white" }}>A3F9</div></div>
            </div>
          </div>
          <div style={{ background:"rgba(255,255,255,0.97)", padding:"0.75rem", display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
            <div style={{ fontSize:"0.4rem", fontWeight:700, color:"#94A3B8", letterSpacing:"0.15em" }}>KASADA OKUTUN</div>
...
const faqData = [
  { q:"Kurulum zor mu?", a:"Hayır. 5 dakikada kurulur. QR standı yerleştirirsiniz, sistem hazır. Teknik bilgi gerekmez." },
  { q:"Uygulama indirmek gerekiyor mu?", a:"Hayır. Müşteri herhangi bir uygulama indirmez. QR kodu okutması yeterli. Tarayıcıda açılır." },
  { q:"SMS sistemi yasal mı?", a:"Evet. Sadece izin veren müşterilere mesaj gönderilir. KVKK tam uyumlu sistemle çalışıyoruz." },
  { q:"5.000 TL'ye ne dahil?", a:"1 yıl tam erişim, kurulum, QR stand, panel, kampanya sistemi, SMS modülü ve teknik destek dahildir." },
  { q:"Deneme yapabilir miyim?", a:"Evet. 7 gün boyunca tüm özellikleri ücretsiz kullanabilirsiniz. Kart gerekmez." },
];
...
export default function LandingPage() {
  ...
  return (
    <div style={{ fontFamily:"'Inter',-apple-system,sans-serif", color:D, background:"white", overflowX:"hidden" }}>
      ...
      {/* NAV */}
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:100, background:scrolled?"rgba(255,255,255,0.92)":"transparent", backdropFilter:scrolled?"blur(12px)":"none", borderBottom:scrolled?`1px solid ${B}`:"1px solid transparent", transition:"all 0.3s" }}>
      ...
      <section style={{ minHeight:"100vh", display:"flex", alignItems:"center", paddingTop:"4rem" }}>
        <div style={{ maxWidth:1100, margin:"0 auto", padding:"4rem 1.5rem", display:"grid", gridTemplateColumns:"1fr 1fr", gap:"3rem", alignItems:"center" }}>
          <div>
            ...
            <h1 style={{ fontSize:"clamp(2.25rem,4.5vw,3.5rem)", fontWeight:900, lineHeight:1.1, letterSpacing:"-0.03em", margin:"0 0 1.25rem" }}>
              Müşterilerinizi<br/>
            ...
            <p style={{ fontSize:"1.05rem", color:G, lineHeight:1.75, margin:"0 0 2rem", maxWidth:460 }}>
              QR tabanlı dijital sadakat sistemi ile tekrar satışlarınızı artırın. Kağıt kart yok. Uygulama yok. Sadece QR.
            </p>
      ...
      {/* TRUST BAR */}
      <section style={{ background:L, borderTop:`1px solid ${B}`, borderBottom:`1px solid ${B}` }}>
        <div style={{ maxWidth:900, margin:"0 auto", padding:"1.75rem 1.5rem", display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"1rem", textAlign:"center" }}>
      ...
      <section style={{ padding:"5rem 1.5rem" }}>
        <div style={{ maxWidth:860, margin:"0 auto", textAlign:"center" }}>
          ...
          <h2 style={{ fontSize:"clamp(1.6rem,3.5vw,2.25rem)", fontWeight:800, letterSpacing:"-0.02em", margin:"0 0 1rem" }}>Müşterileriniz geliyor...<br/>ama geri gelmiyor mu?</h2>
          <p style={{ color:G, fontSize:"1rem", marginBottom:"2.5rem", lineHeight:1.7 }}>Tekrar satış, yeni müşteri kazanmaktan 5x daha ucuzdur. Ama çoğu işletme bunu ölçemiyor bile.</p>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"1rem", textAlign:"left" }}>
            {[{i:"📋",t:"Kağıt Kartlar Kaybolur",d:"Müşteri kartı kaybeder. Sadakat kayıt sıfırlanır. Gelir fırsatı kaçar."},{i:"👻",t:"Müşteri Takibi Yok",d:"Kim 3 aydır gelmedi? Bilinmiyor. Geri çağırma imkânı yok."},{i:"📉",t:"Tekrar Satış Yok",d:"Sadakat sistemi olmadan her satış sanki ilk satış gibi maliyetlidir."}].map(p=>(
      ...
      {/* HOW IT WORKS */}
      <section id="nasil" style={{ background:L, borderTop:`1px solid ${B}`, padding:"5rem 1.5rem" }}>
        <div style={{ maxWidth:860, margin:"0 auto", textAlign:"center" }}>
          <div style={{ fontSize:"0.7rem", fontWeight:700, color:I, letterSpacing:"0.15em", marginBottom:"0.75rem" }}>ÇÖZÜM</div>
          <h2 style={{ fontSize:"clamp(1.6rem,3.5vw,2.25rem)", fontWeight:800, letterSpacing:"-0.02em", margin:"0 0 2.5rem" }}>Tek QR ile sadakati<br/>otomatik hale getirin</h2>
      ...
      {/* FEATURES */}
      <section id="ozellikler" style={{ padding:"5rem 1.5rem" }}>
        <div style={{ maxWidth:860, margin:"0 auto", textAlign:"center" }}>
          <div style={{ fontSize:"0.7rem", fontWeight:700, color:I, letterSpacing:"0.15em", marginBottom:"0.75rem" }}>ÖZELLİKLER</div>
          <h2 style={{ fontSize:"clamp(1.6rem,3.5vw,2.25rem)", fontWeight:800, letterSpacing:"-0.02em", margin:"0 0 2.5rem" }}>İhtiyacınız olan her şey<br/>tek sistemde</h2>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"1rem", textAlign:"left" }}>
            {[{i:"📱",t:"QR ile Anında Giriş",d:"Müşteri göster, kasiyer okut. Saniyeler içinde damga eklenir."},{i:"⭐",t:"Dijital Damga Sistemi",d:"Kağıt kart yok, kayıp yok, dolandırıcılık yok."},{i:"📊",t:"Anlık Analitik Panel",d:"Günlük müşteri ve damga sayısı. Her şey tek ekranda."},{i:"📲",t:"SMS Geri Çağırma",d:"2 hafta gelmeyen müşteriye izinli hatırlatma."},{i:"🎁",t:"Kampanya Oluşturma",d:"7 kahve = 1 bedava. Sınırsız kampanya tanımlayın."},{i:"🔒",t:"KVKK Uyumlu",d:"Müşteri verileri şifreli. Tüm yasal gerekliliklere uygun."}].map(f=>(
      ...
      {/* ROI */}
      <section style={{ background:`linear-gradient(135deg,${I},${P})`, padding:"5rem 1.5rem" }}>
        <div style={{ maxWidth:760, margin:"0 auto", textAlign:"center" }}>
          ...
          <h2 style={{ fontSize:"clamp(1.6rem,3.5vw,2.25rem)", fontWeight:800, color:"white", margin:"0 0 1rem", letterSpacing:"-0.02em" }}>Sadakat sistemi size<br/>para kazandırır</h2>
          <p style={{ color:"rgba(255,255,255,0.75)", fontSize:"1rem", marginBottom:"2.5rem", lineHeight:1.7 }}>Tekrar gelen müşteri edinimi, yeni müşteriden 5x daha ucuzdur. Sisteminiz bu farkı yaratır.</p>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"1rem" }}>
            {[{n:"%28",l:"Daha Fazla Tekrar Ziyaret",s:"tahmini ortalama"},{n:"5x",l:"Daha Ucuz Müşteri Elde Tutma",s:"yeni müşteriye göre"},{n:"↑30%",l:"Müşteri Yaşam Boyu Değeri",s:"sadakat sistemli işletmeler"}].map(r=>(
              <div key={r.n} style={{ background:"rgba(255,255,255,0.1)", border:"1px solid rgba(255,255,255,0.15)", borderRadius:16, padding:"1.5rem" }}>
                ...
                <div style={{ fontWeight:700, color:"white", fontSize:"0.85rem", margin:"0.25rem 0 0.25rem" }}>{r.l}</div>
      ...
      {/* PRICING */}
      <section id="fiyat" style={{ padding:"5rem 1.5rem", background:L, borderTop:`1px solid ${B}` }}>
        <div style={{ maxWidth:480, margin:"0 auto", textAlign:"center" }}>
          <div style={{ fontSize:"0.7rem", fontWeight:700, color:I, letterSpacing:"0.15em", marginBottom:"0.75rem" }}>FİYATLANDIRMA</div>
          <h2 style={{ fontSize:"clamp(1.6rem,3.5vw,2.25rem)", fontWeight:800, letterSpacing:"-0.02em", margin:"0 0 0.75rem" }}>Basit fiyat, güçlü sistem</h2>
          <p style={{ color:G, marginBottom:"2rem", fontSize:"0.95rem" }}>Her şey dahil. Gizli ücret yok.</p>
          <div style={{ background:"white", border:`2px solid ${I}`, borderRadius:24, padding:"2.5rem 2rem", boxShadow:"0 8px 32px rgba(99,102,241,0.15)" }}>
      ...
      <section style={{ padding:"5rem 1.5rem" }}>
        <div style={{ maxWidth:640, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:"2.5rem" }}>
            ...
            <h2 style={{ fontSize:"clamp(1.5rem,3vw,2rem)", fontWeight:800, letterSpacing:"-0.02em" }}>Sık sorulan sorular</h2>
          </div>
      ...
      {/* FINAL CTA */}
      <section style={{ padding:"5rem 1.5rem", background:D }}>
        <div style={{ maxWidth:600, margin:"0 auto", textAlign:"center" }}>
          <h2 style={{ fontSize:"clamp(1.75rem,4vw,2.5rem)", fontWeight:900, color:"white", letterSpacing:"-0.02em", margin:"0 0 1rem" }}>
      ...
      {/* FOOTER */}
      <footer style={{ background:D, borderTop:"1px solid rgba(255,255,255,0.07)", padding:"1.5rem" }}>
